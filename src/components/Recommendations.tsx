    ...
    <Card>
      ...
      <div className="mb-4 grid gap-3 md:grid-cols-2">
        ...
        <select className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm" value={sortBy} onChange={(event) => setSortBy(event.target.value)}>
          <option value="priority">Sort: priority</option><option value="journey">Sort: journey</option><option value="title">Sort: title</option>
        </select>
      </div>
  ...
  return (
    <Card>
      <SectionTitle eyebrow="Action checklist" title={`Explicit Bodhi action checklist (${filtered.length}/${report.actionChecklist.length})`}>
        The workflow emits generic checklist actions with different target URLs/source types. This view exposes the target, category and workstream so the backlog is not a wall of repeated text.
      </SectionTitle>
      <div className="mb-4 grid gap-3 md:grid-cols-2 lg:grid-cols-4">
        ...
        <select className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm" value={sortBy} onChange={(event) => setSortBy(event.target.value)}>
          <option value="priority">Sort: priority</option><option value="target">Sort: target</option><option value="category">Sort: category</option>
        </select>
      </div>
        ...
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          <thead>
            <tr className="text-left text-xs uppercase tracking-wide text-slate-500">
              <th className="px-3 py-3">Action</th><th className="px-3 py-3">Target</th><th className="px-3 py-3">Category / source type</th><th className="px-3 py-3">Owner</th><th className="px-3 py-3">Priority</th><th className="px-3 py-3">Effort</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Workstream</th>
            </tr>
          </thead>
