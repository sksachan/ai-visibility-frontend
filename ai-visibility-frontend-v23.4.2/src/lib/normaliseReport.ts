import type { ActionItem, CitationExample, OwnedPage, QueryDiagnostic, RecommendationModule, ReportBundle, Severity, SiteStandards, SourceTypeCount, Status } from '../types/report';
...
function booleanFrom(value: unknown): boolean {
  ...
  if (typeof value === 'string') return ['true', 'present', 'available', 'yes', '1'].includes(value.trim().toLowerCase());
  return false;
}
...
function pageTech(page: AnyRecord): { jsonLdPresent: boolean; jsonLdBlockCount: number; schemaTypes: string[]; canonicalPresent: boolean; metaDescriptionPresent: boolean } {
  const tech = asRecord(page.technical_signals);
  const structured = asRecord(page.structured_data);
  const metadata = asRecord(page.metadata);
  const schemaTypes = Array.from(new Set([
    ...stringArray(page.schema_types_detected),
    ...stringArray(page.schema_types),
    ...stringArray(tech.schema_types),
    ...stringArray(structured.schema_types),
    ...stringArray(metadata.schema_types)
  ]));
  const jsonLdBlockCount = asNumber(firstDefined(page.json_ld_block_count, page.schema_block_count, tech.json_ld_block_count, structured.json_ld_block_count, metadata.schema_block_count));
  const jsonLdPresent = booleanFrom(firstDefined(page.json_ld_present, page.schema_json_ld, tech.json_ld_present, structured.json_ld_present, metadata.json_ld_present)) || jsonLdBlockCount > 0 || schemaTypes.length > 0;
  return {
    jsonLdPresent,
    jsonLdBlockCount,
    schemaTypes,
    canonicalPresent: booleanFrom(firstDefined(page.canonical_present, tech.canonical_present, page.canonical_url, metadata.canonical)),
    metaDescriptionPresent: booleanFrom(firstDefined(page.meta_description_present, tech.meta_description_present, page.meta_description, page.description, metadata.description))
  };
}

function normaliseSiteStandards(raw: unknown, ownedPages: OwnedPage[]): SiteStandards {
  const source = asRecord(raw);
  const signals = asRecord(source.signals);
  const robots = asRecord(source.robots_txt);
  const llms = asRecord(source.llms_txt);
  const jsonLdPages = ownedPages.filter((page) => page.technicalSignals?.jsonLdPresent).length;
  const total = ownedPages.length;
  const robotsTxtStatus = asString(firstDefined(source.robots_txt_status, robots.status, signals.robots_available === true ? 'present' : undefined), 'not supplied');
  const llmsTxtStatus = asString(firstDefined(source.llms_txt_status, llms.status, signals.llms_txt_available === true ? 'present' : undefined), 'not supplied');
  return {
    robotsTxtStatus: robotsTxtStatus === 'available' ? 'present' : robotsTxtStatus,
    robotsTxtUrl: asString(firstDefined(source.robots_txt_url, robots.resolved_url, robots.url)),
    llmsTxtStatus: llmsTxtStatus === 'available' ? 'present' : llmsTxtStatus,
    llmsTxtUrl: asString(firstDefined(source.llms_txt_url, llms.resolved_url, llms.url)),
    jsonLdPages,
    ownedPagesTotal: total,
    jsonLdCoveragePct: total ? Math.round((jsonLdPages / total) * 1000) / 10 : 0
  };
}
...
function mapOwnedPages(cmsPayload: AnyRecord): OwnedPage[] {
  return asArray<AnyRecord>(firstDefined(cmsPayload.pages, cmsPayload.owned_readiness, cmsPayload.owned_pages, cmsPayload.rows)).map((page, index) => {
    ...
    return {
      ...
      technicalSignals: pageTech(page)
    };
  });
}
...
function mapFrontendPreviewBundle(source: AnyRecord): ReportBundle | null {
  ...
  const hygiene = normaliseSiteStandards(firstDefined(source.site_standards, asRecord(source.evidence_quality).site_standards, source.ai_discoverability_hygiene), ownedPages);
  ...
  return {
    ...
    aiDiscoverabilityHygiene: hygiene,
...
export function normaliseReport(raw: unknown): ReportBundle {
  ...
  const hygiene = normaliseSiteStandards(firstDefined(parsePreviewTile(run, 'site_standards'), asRecord(parsePreviewTile(run, 'bodhi_input_bundle_full')).site_standards, asRecord(parsePreviewTile(run, 'bodhi_input_bundle')).site_standards, asRecord(executivePayload).site_standards), ownedPages);
