...
export function ExecutiveReport({ report }: { report: ReportBundle }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {report.aiDiscoverabilityHygiene ? (
        <Card>
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-4">
            <MetricCard label="Robots.txt" value={report.aiDiscoverabilityHygiene.robotsTxtStatus} note={report.aiDiscoverabilityHygiene.robotsTxtUrl || 'Explicit site standard field'} />
            <MetricCard label="LLMs.txt" value={report.aiDiscoverabilityHygiene.llmsTxtStatus} note={report.aiDiscoverabilityHygiene.llmsTxtUrl || 'Explicit site standard field'} />
            <MetricCard label="JSON-LD/schema coverage" value={`${report.aiDiscoverabilityHygiene.jsonLdPages}/${report.aiDiscoverabilityHygiene.ownedPagesTotal} pages · ${report.aiDiscoverabilityHygiene.jsonLdCoveragePct}%`} note="Explicit page technical signals" />
            <MetricCard label="Priority" value={report.aiDiscoverabilityHygiene.jsonLdCoveragePct < 50 ? 'high' : 'medium'} note="Do not infer from GEO score" />
          </div>
        </Card>
      ) : null}
